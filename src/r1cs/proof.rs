...
impl R1CSProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof, R1CSError> {
        ...
        let t_x = Scalar::from_canonical_bytes(read32!()).into_option().ok_or(R1CSError::FormatError)?;
        let t_x_blinding = Scalar::from_canonical_bytes(read32!()).into_option().ok_or(R1CSError::FormatError)?;
        let e_blinding = Scalar::from_canonical_bytes(read32!()).into_option().ok_or(R1CSError::FormatError)?;
